...
class FuncSignature:
    ...
    def __post_init__(self):
        ...
        def get_error_msg(req_or_resp: str, reason: str):
            raise TypeError(
                f"Error on FuncSignature {req_or_resp} type for method {self.method_name}. {reason}"
            )
        ...
        if not isinstance(self.request_type, type):  # type: ignore
            get_error_msg(
                "Request", f"request_type is not a type: {type(self.request_type)}"
            )

        if not issubclass(self.request_type, BaseMessage):
            get_error_msg(
                "Request",
                f"request_type is not a BaseMessage: {type(self.request_type)}",
            )

        if not isinstance(self.response_type, type):
            get_error_msg(
                "Response", f"response_type is not a type: {type(self.response_type)}"
            )

        if not issubclass(self.response_type, BaseMessage):
            get_error_msg(
                "Response",
                f"response_type is not a BaseMessage: {type(self.response_type)}",
            )
...
def make_service_template(
    service_name: str, funcs: Iterable[FuncSignature]
) -> ServiceTemplate:
